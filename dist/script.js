    ...
    value: function initMask() {
      ...
      function createMask(event) {
        ...
        this.value = matrix.replace(/./g, function (a) {
          return /[_\d]/.test(a) && i < val.length ? val.charAt(i++) : i >= val.length ? '' : a; //Маска перебора матрици
        });
  ...
  _createClass(VideoPlayer, [{
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== "true") {
            ...
            if (document.querySelector('iframe#frame')) {
              //проверка на существующий iframe
              _this.overlay.style.display = 'flex'; //если есть, открываем его, без проверки будет создан новый

              if (_this.path !== btn.getAttribute('data-url')) {
                //и будет звук на фоне если закрыть видео
                _this.path = btn.getAttribute('data-url'); //Проверяем тыкнул ли пользователь в другую кнопку

                _this.player.loadVideoById({
                  videoId: _this.path
                }); //loadVideoById - смотри API youtube

              }
            } else {
              _this.path = btn.getAttribute('data-url'); // если нету iframe, мы его создаем тут
    ...
    value: function bindCloseBtn() {
      ...
      this.close.addEventListener('click', function () {
        ...
        _this2.player.stopVideo(); //stopVideo функция из АРІ youtube
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); //tue дает нам глубокое копирование елемента
